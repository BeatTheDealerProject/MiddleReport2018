\chapter{シミュレーション}
\section{概要}
この章ではブラックジャックの戦略の検証を行う際に設定した仮説とその検証手順、シミュレータの実装のそれぞれの詳細について順に述べていく。
\bunseki{※尾崎拓海}
...
class GamePlayer:
    ...
    def __init__(self):
        self.cards = []  # 参加者の手札
        self.total = 0  # 参加者の手札の合計値
        self.acetotal = 0  # 参加者の手札に含まれるAの枚数
        self.usedace = 0  # 1として数えたAの枚数
        self.burst = False  # バーストしているかどうか
        self.naturalbj = False  # ナチュラルブラックジャックを満たしているかどうか
        self.normalbj = False  # 手札の合計値が21になっているかどうか
